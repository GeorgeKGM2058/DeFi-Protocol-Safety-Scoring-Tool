...
The code first grabs live numbers from CoinGecko (token metrics), DefiLlama (TVL + chains), GitHub (commit activity). 
It also ncludes a fallback cache so it doesn’t crash if an API suddenly doesn't work. 
...
Finally it produces a proper PDF with detailed summary tables, a coloured risk heatmap and a TVL chart for each protocol (includes simple 7 day forecast).

Added detailed comments where required and the weights are easily customizable for faster tweaking in the future.


## Commentary for the report summary example 
...
- Hit “Run all” to run all cells. It should execute everything seemlessly. The last cell will generate the report pdf and auto-download it, also available in the files on the left. Normally done in less than 30–90 seconds.
...
- DefiLlama: If TVL falls back to the hardcoded values, simply re-run the cell. It usually resolves on retry.
- Github apparently is at 60 requests per hour. We only do 9 calls total (3 per protocol) so you’re usually fine.


## Exact data sources

Prices and metrics from CoinGecko API. TVL and chain breakdown from DefiLlama API. 
